...
'[DANO] #1
Public Const MaxLongLongValue As Long = &H7FFFFFFF
...
Public Function ConvertLongToDateStr(ByVal secs As Long) As String
    Dim tz As TIME_ZONE_INFORMATION
    
    If secs > 0 Then
        ConvertLongToDateStr = Str(DateAdd("s", -(tz.Bias + tz.DaylightBias * IIf(GetTimeZoneInformation(tz) = 2, 1, 0)) * 60, _
                                    DateAdd("s", secs, DateSerial(1970, 1, 1))))
    Else
...
End Function
'[DANO] #3
Public Function LongMaxStr(longVal As Long) As String
